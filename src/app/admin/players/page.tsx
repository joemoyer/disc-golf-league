  ...
  return (
    <section className="space-y-4">
      ...
      <form action={createPlayer} className="grid grid-cols-2 gap-2 rounded border bg-white p-3">
        ...
        <input className="rounded border p-2" name="rating" type="number" placeholder="rating" />
        <button className="col-span-2 rounded bg-slate-900 px-3 py-2 text-white">Add Player</button>
      </form>
      <table className="w-full border bg-white text-sm">
        <thead><tr><th className="p-2 text-left">Name</th><th className="p-2 text-left">Email</th><th className="p-2 text-left">Rating</th></tr></thead>
        <tbody>
          {players.map((p) => (
            <tr key={p.id} className="border-t"><td className="p-2">{p.displayName}</td><td className="p-2">{p.email ?? "-"}</td><td className="p-2">{p.rating ?? "-"}</td></tr>
          ))}
        </tbody>
      </table>
    </section>
